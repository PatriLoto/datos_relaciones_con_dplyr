...
-------------------------------
 ...
 Esta restricción se expresa de la forma by = "key". 
 ...
 *  Un vector de caracteres con nombres, by = c("a"="b"), by = c("a" = "b"). Esto va a unir la variable a en 
...
Entonces, esta pequeña tabla que compara las sentencias de dplyr que estuvimos viendo con la de merge_element(
...
SQL soporta un rango más amplio de tipos de uniones que dplyr, 
  ya que puedes conectar tablas usando restricciones distintas a las de igualdad.
Paquete para ejecutar sentencias desde R: sqldf
...
destinos_populares <- vuelos %>%
  ...
  head(10)
toma los diez lugares con mayor catidad de apariciones
...
anti_join()
mantiene las filas que no tienen coincidencias:
