...
* `vuelos` se conecta con `clima` a través de las variables origen (es decir,la ubicación) más `anio`, `mes`, `dia` y `hora` (el `horario`).
...
* Una **clave primaria** identifica únicamente una observación en su propia tabla.
  Por ejemplo, `aviones$codigo_cola` es una clave primaria, ya que identifica de manera única cada avión en la tabla `aviones`.

* Una **clave foránea** únicamente identifica una observación en otra tabla.
  Por ejemplo, `vuelos$codigo_cola` es una clave foránea, ya que aparece en la tabla `vuelos`, en la que une cada vuelo con un único avión.
...
.pull.left[.pad1[.font2[#### Tipos:
**unión interior**]]]]]
.pull.right[.pad1[.font2[#### **unión exterior**]]]]]

**unión interior**                        **unión exterior**
--------------------------------------------------------------
mantiene las observaciones       Una  mantiene las observaciones
que aparecen en ambas tablas.   que aparecen en al menos una de las                                 tablas
