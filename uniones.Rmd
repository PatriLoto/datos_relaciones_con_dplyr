...
¿Cómo lo hacemos? Combinamos los datos de `vuelos2` y `aerolinas` con un `left_join()` (*union_izquierda*):

```{r vuelos_aerolinea, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}

vuelos2 %>%
  select(-origen, -destino) %>%
  left_join(aerolineas, by = "aerolinea")%>% head(10)#<<
...
El resultado de unir `vuelos2` y `aerolineas` es la inclusión de una variable adicional: `nombre` en la primera tabla. Esta es la razón de que llamemos unión de transformación a este tipo de unión.
...
```
Representación gráfica de las uniones exteriores_
